...
namespace Kaisa.Digivice
{
    public class DebugManager : MonoBehaviour
    {
        ...
        private bool consoleActivated = true;
        ...
        public string AnalyzeCommand(string command)
        {
            ...
            if (command.StartsWith("/setspiritpower99"))
            {


                SavedGame.SpiritPower = 99;


            }
            if (command.StartsWith("/distancenew0"))
            {

                SavedGame.CurrentDistance = 2;
            ...
            if (command.StartsWith("/distancenew"))
            {
                string[] args = command.Split(' ');
                if (args.Length == 2)
                {
