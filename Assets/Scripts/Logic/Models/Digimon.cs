    ...
    public class Digimon {
        ...
        public int MaxExtraLevel {
            get {
                ...
                else if (stage == Stage.Spirit || (int)stage ==10 ) maxLevel = 0;
                else maxLevel = Mathf.CeilToInt(baseLevel * 1.5f);
                return maxLevel - baseLevel;
            }
        }
        public static int Digicost(int playerLevel){
            float currentCost = 20 * Mathf.Pow(0.5f, playerLevel / 20);
        ...
        public int GetSpiritCost(int playerLevel) {
            ...
            //     decay = 20;
            // }
            else if (stage == Stage.Spirit || (int)stage==10) {
        ...
        public float GetObeyChance(int playerLevel) {
            ...
            int currentBaseLevel = (stage == Stage.Spirit || (int)stage==10) ? GetBossLevel(playerLevel) : baseLevel;
        ...
        public MutableCombatStats GetBossStats(int playerLevel) {
            ...
            if (stage == Stage.Spirit || (int)stage == 10) {
        ...
        private int GetStatAsRegularBoss(int stat, int bossLevel) {
            float val = ((((bossLevel * 0.01f + 0.01f)) * stat)) - (bossLevel * 0.77f);
            float riggedStat = Mathf.Ceil(val) < stat ? Mathf.Ceil(val) : stat;
            return Mathf.RoundToInt(riggedStat);
        }
